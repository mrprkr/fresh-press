...
import { steps, PHASE_META, PHASE_ORDER, type Step, type StepStatus, type Phase } from './steps.js'
...
type Row = { type: 'header'; phase: Phase } | { type: 'step'; step: Step }
      ...
      <Box flexDirection="column" paddingX={2}>
        {PHASE_ORDER.map((phase, i) => (
          <Box key={phase}>
            ...
            <Text color={phaseColor(phase)}>{PHASE_META[phase].label}</Text>
        ...
        if (row.type === 'header') {
          ...
          return (
            <Box key={`h-${row.phase}`} marginTop={i === 0 ? 0 : 1}>
              ...
              <Text color={allSelected ? 'green' : someSelected ? 'yellow' : 'gray'}>{check}</Text>
...
function ConfirmScreen({
  ...
  return (
    <Box flexDirection="column" padding={1}>
      <Text color="cyan" bold>
        Ready to restore — {selected.size} steps:
      </Text>
      <Newline />
...
function RunningScreen({
  ...
  const currentPhase = steps.find((s) => stepStates.get(s.id) === 'running')?.phase
...
export default function App() {
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set(steps.map((s) => s.id)))
